    ...
    <details className="w-full" open>
      ...
      <ul className="my-3 flex flex-col gap-1">
        {categories.map((category) => {
          ...
          return (
            <a key={category} href={`/catalog/${category}`} className="inline">
              <li className="ml-5 capitalize flex justify-between text-sm">
                <p>{category}</p>
                <p>
                  {category === "all" ? products.length : categoryCount.length}
                </p>
              </li>
            </a>
          );
    ...
    <details className="w-full" open>
      ...
      <ul className="my-3 flex flex-col gap-1">
        {priceRanges.map((range) => (
          <li key={range.toString()} className="ml-5 capitalize flex text-sm">
            <p>${range[0]}</p>
